...
SELECT email FROM salespeople WHERE 'Southwest' IN (region) OR 'Northwest' IN (region);
...
SELECT email, first_name, last_name FROM salespeople WHERE ('Northwest' IN (region) OR 'Southwest' IN (region)) AND last_name LIKE 'M%';
